...
/*   Updated: 2023/05/27 18:03:06 by kgebski          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE < 1)
		return (0);
	saved_str[fd] = update_saved_str(saved_str[fd], fd);
	if (!saved_str[fd])
		return (0);
	result = get_result(saved_str[fd]);
	saved_str[fd] = get_rest_of_str(saved_str[fd]);
	return (result);
}
